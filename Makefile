...
deps:
	GOPRIVATE=github.com/HealthyFeelingNL
	go mod tidy
...
mocks:
	...
	mkdir -p mocks
	$$GOPATH/bin/mockgen -package=mocks github.com/HealthyFeelingNL/go-kit/wrappers/http Client > mocks/http_mocks.go
	$$GOPATH/bin/mockgen -package=mocks net/http Handler > mocks/http_handler_mocks.go
	$$GOPATH/bin/mockgen -package mocks github.com/HealthyFeelingNL/go-kit/encoding Encoder > mocks/encoding_mocks.go
...
lint:
	$$GOPATH/bin/revive -config vendor/github.com/HealthyFeelingNL/go-kit/revive.toml -formatter friendly $$(find . -name "*.go" | grep -v vendor | grep -v service | grep -v mocks | uniq)
